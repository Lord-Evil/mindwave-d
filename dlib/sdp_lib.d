 ...
 * SDP lists
 */
alias sdp_list_func_t = void function (void*, void*);
alias sdp_free_func_t = void function (void*);
alias sdp_comp_func_t = int function (const(void)*, const(void)*);
...
int sdp_list_len (const(sdp_list_t)* list);

sdp_list_t* sdp_list_find (sdp_list_t* list, void* u, sdp_comp_func_t f);

void sdp_list_foreach (sdp_list_t* list, sdp_list_func_t f, void* u);

/*
 * Values of the flags parameter to sdp_record_register
 */
enum SDP_RECORD_PERSIST = 0x01;
enum SDP_DEVICE_RECORD = 0x02;

/*
 * Values of the flags parameter to sdp_connect
 */
enum SDP_RETRY_IF_BUSY = 0x01;
enum SDP_WAIT_ON_CLOSE = 0x02;
enum SDP_NON_BLOCKING = 0x04;
enum SDP_LARGE_MTU = 0x08;
...
struct sdp_session_t
{
    ...
    int local;
    int flags;
    ushort tid; /* Current transaction ID */
    void* priv;
}

enum sdp_attrreq_type_t
{
    /*
 ...
 * 	    - I/O error
 */
alias sdp_callback_t = void function (ubyte type, ushort status, ubyte* rsp, size_t size, void* udata);
 ...
 */
sdp_session_t* sdp_connect (const(bdaddr_t)* src, const(bdaddr_t)* dst, uint flags);
 ...
 */
sdp_session_t* sdp_create (int sk, uint flags);
int sdp_get_error (sdp_session_t* session);
int sdp_process (sdp_session_t* session);
int sdp_set_notify (sdp_session_t* session, void function () func, void* udata);

int sdp_service_search_async (sdp_session_t* session, const(sdp_list_t)* search, ushort max_rec_num);
int sdp_service_attr_async (sdp_session_t* session, uint handle, sdp_attrreq_type_t reqtype, const(sdp_list_t)* attrid_list);
int sdp_service_search_attr_async (sdp_session_t* session, const(sdp_list_t)* search, sdp_attrreq_type_t reqtype, const(sdp_list_t)* attrid_list);

ushort sdp_gen_tid (sdp_session_t* session);

/*
 * find all devices in the piconet
 */
int sdp_general_inquiry (inquiry_info* ii, int dev_num, int duration, ubyte* found);

/* flexible extraction of basic attributes - Jean II */
int sdp_get_int_attr (const(sdp_record_t)* rec, ushort attr, int* value);
int sdp_get_string_attr (const(sdp_record_t)* rec, ushort attr, char* value, int valuelen);

/*
 * Basic sdp data functions
 */
sdp_data_t* sdp_data_alloc (ubyte dtd, const(void)* value);
sdp_data_t* sdp_data_alloc_with_length (ubyte dtd, const(void)* value, uint length);
void sdp_data_free (sdp_data_t* data);
sdp_data_t* sdp_data_get (const(sdp_record_t)* rec, ushort attr_id);
...
int sdp_attr_add (sdp_record_t* rec, ushort attr, sdp_data_t* data);
void sdp_attr_remove (sdp_record_t* rec, ushort attr);
void sdp_attr_replace (sdp_record_t* rec, ushort attr, sdp_data_t* data);
int sdp_set_uuidseq_attr (sdp_record_t* rec, ushort attr, sdp_list_t* seq);
int sdp_get_uuidseq_attr (const(sdp_record_t)* rec, ushort attr, sdp_list_t** seqp);
 ...
 */
int sdp_attr_add_new (sdp_record_t* rec, ushort attr, ubyte dtd, const(void)* p);
 ...
 * conforms to.
 */
int sdp_set_service_classes (sdp_record_t* rec, sdp_list_t* seq);
 ...
 */
int sdp_get_service_classes (const(sdp_record_t)* rec, sdp_list_t** seqp);
 ...
 */
int sdp_set_browse_groups (sdp_record_t* rec, sdp_list_t* seq);
 ...
 */
int sdp_set_service_ttl (sdp_record_t* rec, uint ttl);
 ...
 * the record.
 */
int sdp_set_record_state (sdp_record_t* rec, uint state);
 ...
 */
int sdp_set_service_avail (sdp_record_t* rec, ubyte avail);
 ...
 */
int sdp_service_search_req (sdp_session_t* session, const(sdp_list_t)* search, ushort max_rec_num, sdp_list_t** rsp_list);
 ...
 */
sdp_record_t* sdp_service_attr_req (sdp_session_t* session, uint handle, sdp_attrreq_type_t reqtype, const(sdp_list_t)* attrid_list);
 ...
 */
int sdp_device_record_register_binary (sdp_session_t* session, bdaddr_t* device, ubyte* data, uint size, ubyte flags, uint* handle);
int sdp_device_record_register (sdp_session_t* session, bdaddr_t* device, sdp_record_t* rec, ubyte flags);
int sdp_record_register (sdp_session_t* session, sdp_record_t* rec, ubyte flags);

/*
 * Unregister a service record.
 */
int sdp_device_record_unregister_binary (sdp_session_t* session, bdaddr_t* device, uint handle);
 ...
 * in an error.)
 */
int sdp_device_record_update_binary (sdp_session_t* session, bdaddr_t* device, uint handle, ubyte* data, uint size);
 ...
 * UUID functions
 */
uuid_t* sdp_uuid16_create (uuid_t* uuid, ushort data);
uuid_t* sdp_uuid32_create (uuid_t* uuid, uint data);
...
int sdp_uuid_extract (const(ubyte)* buffer, int bufsize, uuid_t* uuid, int* scanned);
void sdp_uuid_print (const(uuid_t)* uuid);

enum MAX_LEN_UUID_STR = 37;
enum MAX_LEN_PROTOCOL_UUID_STR = 8;
enum MAX_LEN_SERVICECLASS_UUID_STR = 28;
enum MAX_LEN_PROFILEDESCRIPTOR_UUID_STR = 28;
 ...
 */
int sdp_get_browse_groups (const(sdp_record_t)* rec, sdp_list_t** seqp);
...
int sdp_get_record_state (const(sdp_record_t)* rec, uint* svcRecState);
int sdp_get_service_avail (const(sdp_record_t)* rec, ubyte* svcAvail);
int sdp_get_service_ttl (const(sdp_record_t)* rec, uint* svcTTLInfo);
int sdp_get_database_state (const(sdp_record_t)* rec, uint* svcDBState);

int sdp_get_service_name (const(sdp_record_t)* rec, char* str, int len);

int sdp_get_service_desc (const(sdp_record_t)* rec, char* str, int len);

int sdp_get_provider_name (const(sdp_record_t)* rec, char* str, int len);

int sdp_get_doc_url (const(sdp_record_t)* rec, char* str, int len);

int sdp_get_clnt_exec_url (const(sdp_record_t)* rec, char* str, int len);

int sdp_get_icon_url (const(sdp_record_t)* rec, char* str, int len);
...
sdp_record_t* sdp_extract_pdu (const(ubyte)* pdata, int bufsize, int* scanned);
...
void sdp_set_seq_len (ubyte* ptr, uint length);
void sdp_set_attrid (sdp_buf_t* pdu, ushort id);
void sdp_append_to_pdu (sdp_buf_t* dst, sdp_data_t* d);
void sdp_append_to_buf (sdp_buf_t* dst, ubyte* data, uint len);

int sdp_gen_pdu (sdp_buf_t* pdu, sdp_data_t* data);
int sdp_gen_record_pdu (const(sdp_record_t)* rec, sdp_buf_t* pdu);

int sdp_extract_seqtype (const(ubyte)* buf, int bufsize, ubyte* dtdp, int* size);

sdp_data_t* sdp_extract_attr (const(ubyte)* pdata, int bufsize, int* extractedLength, sdp_record_t* rec);

void sdp_pattern_add_uuid (sdp_record_t* rec, uuid_t* uuid);
void sdp_pattern_add_uuidseq (sdp_record_t* rec, sdp_list_t* seq);

int sdp_send_req_w4_rsp (sdp_session_t* session, ubyte* req, ubyte* rsp, uint reqsize, uint* rspsize);
